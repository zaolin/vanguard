...
// RecoveryCmd manages HOTP recovery for a Vanguard-protected system.
// Without flags, prints recovery instructions. With --enable, generates
// a HOTP seed and stores it in TPM NVRAM. With --disable, removes it.
type RecoveryCmd struct {
	LUKSDevice string `short:"l" help:"LUKS device path (e.g., /dev/nvme0n1p2)"`
	Enable     bool   `help:"Enable HOTP recovery (generate seed, write to TPM NVRAM, display QR code)"`
	Disable    bool   `help:"Disable HOTP recovery (remove seed from TPM NVRAM)"`
	Show       bool   `help:"Show current HOTP seed and QR code (for re-enrollment)"`
	Check      bool   `help:"Verify that HOTP recovery is properly configured and the seed is readable"`
